...
async function performBorrowRequestOperations(
  ...
  try {
    ...
    // 1. A frontend generated alphanumeric code (5-12 chars, case-insensitive)
    // 2. The literal word "confirm" (case-insensitive)
    // 3. A partial match with the book title (case-insensitive, minimum 5 chars overall)
...
export const approveBorrowRequest = async (
  ...
  // Direct implementation without idempotency wrapper to fix 500 error
  return db.transaction(async (tx) => {
    try {
      // Get the request details with book and user info
      const requestDetails = await tx
        .select({
          request: borrowRequests,
          user: {
            id: users.id,
      ...
      // Only decrement at approval time if we did NOT reserve on request
      if (!isFeatureEnabled("RESERVE_ON_REQUEST") && !book.reserveOnRequest) {
        ...
        // Check for negative inventory (safety check)
        if (updateResult[0].availableCopies < 0) {
          await LibraryMetrics.alertInventoryViolation(
            request.bookId,
            updateResult[0].availableCopies
      ...
      // Send email notification (simplified to avoid email errors)
      try {
...
export const rejectBorrowRequest = async (
  ...
  return db.transaction(async (tx) => {
    try {
      ...
      // Send rejection email (simplified to avoid email errors)
      try {
        if (isFeatureEnabled("ENABLE_EMAIL_NOTIFICATIONS")) {
          await sendEmail({
            email: user.email,
